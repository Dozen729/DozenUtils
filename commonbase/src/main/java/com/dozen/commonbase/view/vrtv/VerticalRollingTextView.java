...
public class VerticalRollingTextView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        // 绘制文本
        if (mDataSetAdapter == null) {
            return;
        }

        String text1 = mDataSetAdapter.getItem(mCurrentIndex).getName();
        String text2 = mDataSetAdapter.getItem(mNextIndex).getName();
        ...
        //设置文字居中显示
        float xOff1 = width - getTextWidth(text1, mPaint);
        float xOff2 = width - getTextWidth(text2, mPaint);
        xOff1 = xOff1 > 0 ? xOff1 / 2 : 0;
        xOff2 = xOff2 > 0 ? xOff2 / 2 : 0;

        canvas.drawText(text1, xOff1, mCurrentOffsetY, mPaint);
        canvas.drawText(text2, xOff2, mCurrentOffsetY + mOffset + mTextTopToAscentOffset, mPaint);
    }
